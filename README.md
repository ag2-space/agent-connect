...
are read by nothing in this package. `AGENT_CONNECT_RESULT_DIR` was read here
once, by the outgoing-file staging airlock; that protocol is retired and its
replacement is `AGENT_CONNECT_EGRESS_ROOTS` above.
`AGENT_CONNECT_OUTGOING_MAX_BYTES` went with it — the size ceiling is the relay
client's own, so there is one number rather than two that can disagree.
