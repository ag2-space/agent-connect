...
> **Устарело в части транспорта — и намеренно не переписано.** Это датированная
> запись о состоянии на 2026-08-05, а не действующий контракт; переписать её
> задним числом значило бы подделать источник. С тех пор воркер забрал транспорт
> себе (workspace ADR 0001, `ag2_relay_client` в `relay-client/` этого же
> репозитория): двухпроцессной схемы больше нет, `install.sh` не ставит
> `ag2-sparrow` и не пишет `RELAY_PIP_SPEC`, флаг `--sutando-workspace` снят, а
> переменные `AGENT_CONNECT_TASK_DIR` / `_RESULT_DIR` / `_STATE_DIR` не читаются
> ничем в этом пакете. Всё, что ниже говорит «два процесса», «файловый шов» или
> «`tasks/` → `results/`», описывает то, что было. Действующее состояние —
> `README.md` (§ *Running more than one agent on one machine*) и `CONTEXT.md`.
