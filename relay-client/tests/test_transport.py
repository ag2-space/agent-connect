...
with FakeBroker() as broker:
    ...
    broker.on("GET", "/v1/tasks", status=302,
              headers={"Location": "http://elsewhere.example/v1/tasks"})
    followed = None
    try:
        http.get("/v1/tasks")
    except RelayHTTPError as exc:
    ...
    check(not any("elsewhere.example" in r.headers.get("Host", "")
                  for r in broker.requests), "the bearer never left for the named host")
