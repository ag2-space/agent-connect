...
class TokenSource:
    ...
    def __init__(
        ...
        if raw:
            ...
            raw = held.secret
            self.source = f"the token file {self.token_file}"
        else:
    ...
    def __repr__(self) -> str:  # pragma: no cover — diagnostics only
        return f"<TokenSource {self.base_url} from {self.source}>"
    ...
    def reload(self) -> Rotation:
        ...
        different gateway — in either the combined `url|secret` layout or the
        split token-plus-URL-line layout — the swap is refused and logged, and
        this client keeps polling the gateway it started on.
        """
        ...
        named = (url_from_token or held.url).strip().rstrip("/")
        if named and named != self.base_url:
            reason = (
                f"{self.token_file} names a different gateway "
                f"({redact_url(named)}) than the running one "
                f"({redact_url(self.base_url)}) — a gateway change is a "
                "reconfiguration, not a rotation; restart to move gateways"
            )
